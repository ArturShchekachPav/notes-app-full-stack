...
const deleteNoteById = (req, res, next) => {
  ...
  return dbConnection.query('SELECT * FROM notes WHERE note_id = ?', [noteId])
    .then(note => {
      ...
      return dbConnection.query('DELETE FROM notes WHERE user_id = ?', [noteId])
        .then(deletedNote => res.send(deletedNote[0][0]));
    })
    .catch(next);
};
...
const updateNote = (req, res, next) => {
  const {noteId} = req.params;
  const {newNoteTitle, content, list_id, color_id} = req.body;
  ...
  return dbConnection.query('SELECT * FROM notes WHERE note_id = ?', [noteId])
    .then(note => {
      ...
      return dbConnection.query('UPDATE notes SET note = ?, content = ?, list_id = ?, color_id = ? WHERE id = ?', [newNoteTitle, content, list_id, color_id, noteId])
        .then(() => dbConnection.query('SELECT * FROM notes WHERE note_id = ?', [noteId]))
...
const getNotes = (req, res, next) => {
  return dbConnection.query('SELECT * FROM notes WHERE user_id = ?', [req.user.id])
